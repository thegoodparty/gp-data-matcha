...
from scripts.constants import BASE_POST_PREDICTION_FILTER, EO_POST_PREDICTION_FILTER
...
def test_base_filter_guards_locality_branch_with_district_identifier():
    ...
    rescued by shared city/county tokens alone.
    """
    assert "district_identifier_l" in BASE_POST_PREDICTION_FILTER
    assert "district_identifier_r" in BASE_POST_PREDICTION_FILTER
...
def _row(**overrides):
    """Build a row dict with all columns the BASE filter references."""
    base = {
        "gamma_last_name": 3,
        "gamma_first_name": 3,
        "gamma_email": -1,
...
def test_base_filter_drops_same_locality_different_district():
    ...
    assert _eval_filter(BASE_POST_PREDICTION_FILTER, row) is False


def test_base_filter_keeps_same_locality_matching_district():
    """Same district_identifier + locality-token overlap → allowed."""
    row = _row(district_identifier_l="8", district_identifier_r="8")
    assert _eval_filter(BASE_POST_PREDICTION_FILTER, row) is True


def test_base_filter_keeps_same_locality_one_null_district():
...
def test_base_filter_keeps_strong_office_name_match_despite_district_diff():
